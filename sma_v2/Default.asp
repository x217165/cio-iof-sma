...
***************************************************************************************************
	  ...
	  15-Oct-2015  PJS
	        Added "and AG.ACCESS_GROUP NOT LIKE 'NON-%'" to the access group select
	        This is to ignore groups such as "NON-LCD" and "NON-OPNAS"
	  03-Feb-2016  PJS
	        Removed Back and Forward arrows
...
strUserName = Request.ServerVariables("HEADER_PF_AUTH_SUBJECT")
...
If UCase(Request.QueryString("redir")) = "Y" Then
	...
	strError = strError & "<BR><BR>1 " & Session(strConst_Logon)
	strError = strError & "<BR><BR>2 " & Session("UserAccessLevel")
	strError = strError & "<BR><BR>3 " & Session("username")
...
If (strUserName <> "") Then
	'response.write SMA2 & " " & strUserName & " " & userRoleNames & " " & userPolicyNames 
	...
	'            userRoleNames(UBound(userRoleNames))="NON-LCD"
				  for i = 0 to UBound(userRoleNames)
				     ...
				     'response.write "<p> i=" & i & " strRoles=" & strRoles & "</p>"
				   next
			 else
  ...
  ' response.end
   	...
   	dim strConnect, strSQL, objConn, objPass
        'If strRoles=" GCNET; GONET" then
        'strRoles=" GCNET;GOCWCS;GONET"
        'end if 

	if len(strRoles) > 0  then
	  strSQL = " Select AG.ORACLE_ID from crp_SEC.ACCESS_GROUP_ID ag where oracle_id like 'APP_SMA_%' and AG.ACCESS_GROUP NOT LIKE 'NON-%' group by AG.ORACLE_ID having LISTAGG(upper(AG.access_GROUP), ';') " &_
	...
	'response.write "Conn" & strConstConnectString
    	strConnect = Decrypt(strConstConnectString) 		'match the one in sma_env.inc
'response.write "Conn" & strConnect 
	end if
   ...
   ' response.end
	...
	'strTestUserName = Request.ServerVariables["LOGON_USER"];
	'response.write strTestUserName
