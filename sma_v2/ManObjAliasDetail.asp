...
<!--
*************************************************************************************
* File Name:	ManObjAliasDetail.asp
*
* Purpose:
*
* In Param:
...
**************************************************************************************
       ...
       18-Mar-02	 DTy		Chang CInt to CLng which is causing variable
                                  incompatibility.
**************************************************************************************
-->
...
if strAction = "save" then
	...
	if (strAliasID <> "") and (intAccessLevel and intConst_Access_Update = intConst_Access_Update) then
		...
		cmdUpdateObj.CommandText = "sma_sp_userid.spk_sma_mo_inter.sp_mo_alias_update"
		'create params
		...
		cmdUpdateObj.Parameters.Append cmdUpdateObj.CreateParameter("p_alias", adVarChar, adParamInput, 30, strNameAlias)
		cmdUpdateObj.Parameters.Append cmdUpdateObj.CreateParameter("p_mo_id", adNumeric , adParamInput,, CLng(strMasterID))
	...
	elseif (strAliasID = "") and (intAccessLevel and intConst_Access_Create = intConst_Access_Create) then
		...
		cmdInsertObj.CommandText = "sma_sp_userid.spk_sma_mo_inter.sp_mo_alias_insert"
		'create params
		cmdInsertObj.Parameters.Append cmdInsertObj.CreateParameter("p_user_id", adVarChar , adParamInput, 30, strRealUserID)
		cmdInsertObj.Parameters.Append cmdInsertObj.CreateParameter("p_alias_id", adNumeric , adParamOutput)
		cmdInsertObj.Parameters.Append cmdInsertObj.CreateParameter("p_alias", adVarChar, adParamInput, 30, strNameAlias)
		cmdInsertObj.Parameters.Append cmdInsertObj.CreateParameter("p_mo_id", adNumeric , adParamInput,, CLng(strMasterID))
		'call the update stored proc
		on error resume next
		...
		if strAliasID = "" then
			DisplayError "BACK", "", 2100, "CANNOT DISPLAY NEW ALIAS.", "Most probably the new alias has been saved successfully even if there was an error retrieving the new id. Close the alias window to return to the managed objects screen."
			objConn.Errors.Clear
		end if
		strWinMessage = "Record saved successfully. You can now see the changes you made."
	else
...
if strAction = "delete" then
	...
	if intAccessLevel and intConst_Access_Delete = intConst_Access_Delete then
		...
		cmdDeleteObj.CommandText = "sma_sp_userid.spk_sma_mo_inter.sp_mo_alias_delete"
		'create params                
	...
	if Request("back") = "true" then
		Response.Redirect "manobjalias.asp?ne_id="&strMasterID
	end if
...
if strAction <> "new" then
	sql =	"SELECT "&_
				"NETWORK_ELEMENT_NAME_ALIAS_ID, "&_
				"NETWORK_ELEMENT_ID, "&_
				"NETWORK_ELEMENT_NAME_ALIAS, "&_
				"CREATE_DATE_TIME, "&_
				...
				"RECORD_STATUS_IND "&_
			"FROM CRP.NETWORK_ELEMENT_NAME_ALIAS "&_
			"WHERE NETWORK_ELEMENT_NAME_ALIAS_ID = " & strAliasID
...
function btnNew_click(){
	...
	document.location.href ="ManObjAliasDetail.asp?action=new&masterID="+strMasterID;
}
...
function fct_onDelete(){
	...
	if (confirm('Do you really want to delete this object?')){
		document.location = "ManObjAliasDetail.asp?action=delete&masterID="+strMasterID+"&AliasID="+document.frmAlias.AliasID.value+"&hdnLastUpdate="+escape(document.frmAlias.hdnLastUpdate.value);
	}
...
function body_onUnload(){
	opener.document.frmMODetails.btn_iFrameRefresh.click();
}
</SCRIPT>
</HEAD>
...
<INPUT type=hidden name=AliasID value="<%if strAliasID <> "" then Response.Write rsAlias("NETWORK_ELEMENT_NAME_ALIAS_ID")%>">
<INPUT type=hidden name=MasterID value="<%=strMasterID%>">

<TABLE border=0 width=100%>
<THEAD>
	<TR ><TD colspan=2>Network Element Name Alias Detail</td></tr>
</THEAD>

<TBODY>
<TR>
	<TD ALIGN=RIGHT width=20% NOWRAP>Alias Name</TD>
	<TD width=80%><INPUT size=40 maxlength=30 name=txtNameAlias value="<%if strAliasID <> "" then Response.write rsAlias("NETWORK_ELEMENT_NAME_ALIAS")%>" onchange ="fct_onChange();">
</TR>
</TBODY>
