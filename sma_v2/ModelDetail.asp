...
<%
Const ASP_NAME = "ModelDetail.asp"
Const NO_ID = "null"
Const TABLE = "MODEL"   'used in Javascript for references button and must be uppercase
Const UPDATE_PROC = "sma_sp_userid.spk_sma_asset_inter.sp_model_update"
Const INSERT_PROC = "sma_sp_userid.spk_sma_asset_inter.sp_model_insert"
Const DELETE_PROC = "sma_sp_userid.spk_sma_asset_inter.sp_model_delete"
...
select case Request("hdnFrmAction")
	case "SAVE"
		if strID <> NO_ID then
			...
			if (intAccessLevel and intConst_Access_Update) <> intConst_Access_Update then
				DisplayError "REFRESH", strWinLocation, 0, "UPDATE DENIED", "You don't have access to update models. Please contact your system administrator."
			end if
			...
			if (intAccessLevel and intConst_Access_Create) <> intConst_Access_Create then
				DisplayError "REFRESH", strWinLocation, 0, "INSERT DENIED", "You don't have access to create models. Please contact your system administrator."
			end if
	...
	case "DELETE"
		if (intAccessLevel and intConst_Access_Delete) <> intConst_Access_Delete then
			DisplayError "REFRESH", strWinLocation, 0, "DELETE DENIED", "You don't have access to delete models. Please contact your system administrator"
		end if
...
function form_onSubmit()
{
	...
	if ( ((<%=intAccessLevel%> & <%=intconst_Access_Create%>) == <%=intconst_Access_Create%>) || ( (<%=intAccessLevel%> & <%=intconst_Access_Update%>) == <%=intconst_Access_Update%>) )
	{
		...
		{
			alert("Please type a model description.");
...
<table width="100%" border=0>
	<thead>
		<TR><TD align=left colspan=2>Model Detail</TD>
	</thead>
	<tbody>
		<TR>
			<TD ALIGN=RIGHT NOWRAP>Model<font color="red">*</font></TD>
			<TD>
				<INPUT name=txtDesc style="HEIGHT: 23px; WIDTH: 300px" value="<%if strID <> NO_ID then  Response.Write routineHtmlString(objRS("MODEL_DESC")) else Response.Write null end if%>" onchange ="fct_onChange();">
			</TD>
