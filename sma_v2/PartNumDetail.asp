...
<%
Const ASP_NAME = "PartNumDetail.asp"
Const NO_ID = "null"
Const TABLE = "PART_NUMBER"   'used in Javascript for references button and must be uppercase
Const UPDATE_PROC = "sma_sp_userid.spk_sma_asset_inter.sp_part_number_update"
Const INSERT_PROC = "sma_sp_userid.spk_sma_asset_inter.sp_part_number_insert"
Const DELETE_PROC = "sma_sp_userid.spk_sma_asset_inter.sp_part_number_delete"
...
select case Request("hdnFrmAction")
	case "SAVE"
		if strID <> NO_ID then
			...
			if (intAccessLevel and intConst_Access_Update) <> intConst_Access_Update then
				DisplayError "REFRESH", strWinLocation, 0, "UPDATE DENIED", "You don't have access to update part numbers. Please contact your system administrator."
			end if
			...
			if (intAccessLevel and intConst_Access_Create) <> intConst_Access_Create then
				DisplayError "REFRESH", strWinLocation, 0, "INSERT DENIED", "You don't have access to create part numbers. Please contact your system administrator."
			end if
	...
	case "DELETE"
		if (intAccessLevel and intConst_Access_Delete) <> intConst_Access_Delete then
			DisplayError "REFRESH", strWinLocation, 0, "DELETE DENIED", "You don't have access to delete part numbers. Please contact your system administrator"
		end if
...
if strID <> NO_ID then
	...
	StrSql = " SELECT part_number_id" &_
			 " ,      part_number_desc" &_
			 ...
			 " ,      record_status_ind " &_
			 " FROM   crp.part_number " &_
			 " WHERE part_number_id = " & strID
...
function form_onSubmit()
{
	...
	if ( ((<%=intAccessLevel%> & <%=intconst_Access_Create%>) == <%=intconst_Access_Create%>) || ( (<%=intAccessLevel%> & <%=intconst_Access_Update%>) == <%=intconst_Access_Update%>) )
	{
		...
		{
			alert("Please type a part number description.");
...
<FORM NAME=frmDetail METHOD=POST ACTION="<%=ASP_NAME%>"  onsubmit="form_onSubmit();">
<!-- hidden fields -->
...
<table width="100%" border=0>
	<thead>
		<TR><TD align=left colspan=2>Part Number Detail</TD>
	</thead>
	<tbody>
		<TR>
			<TD ALIGN=RIGHT NOWRAP>Part Number<font color="red">*</font></TD>
			<TD>
				<INPUT name=txtDesc style="HEIGHT: 23px; WIDTH: 300px" value="<%if strID <> NO_ID then  Response.Write routineHtmlString(objRS("PART_NUMBER_DESC")) else Response.Write null end if%>" onchange ="fct_onChange();">
			</TD>
