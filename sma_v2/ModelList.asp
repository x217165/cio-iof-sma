...
'build query
sql = " SELECT model_id" &_
	  " ,      model_desc" &_
	  " FROM   crp.model"

if strCriteria <> "" then
	sql = sql &	" WHERE  UPPER(model_desc) LIKE '" & routineOraString(strCriteria) & "%'"
end if

'order by make description
sql = sql & " ORDER BY UPPER(model_desc)"
...
{
	parent.opener.document.forms[0].hdnModelID.value = ID;
	parent.opener.document.forms[0].txtModelDesc.value = Desc;
	parent.window.close ();
}
