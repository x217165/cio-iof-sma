...
dim strRegion, strSupportGroup, strStatus, strCustServDesc, strObjName, strCustomerName, strCustServID, strActive, hdnManagedObjects
...
hdnManagedObjects = Request("hdnManagedObjects")
...
if strObjName = "" then
	...
	sql = "select "&_
			...
			"CUS.CUSTOMER_NAME, "&_
			"CUS.NOC_REGION_LCODE "
	
	if hdnManagedObjects = "1" then
		sql = sql & ", NE.NETWORK_ELEMENT_NAME "
	end if
	...
	if hdnManagedObjects = "1" then
		sql = sql & ", CRP.MANAGED_CORRELATION MC, CRP.NETWORK_ELEMENT NE "
	end if	
	...
	if hdnManagedObjects = "1" then
		sql = sql & "AND CS.CUSTOMER_SERVICE_ID(+) = MC.CUSTOMER_SERVICE_ID AND MC.NETWORK_ELEMENT_ID = NE.NETWORK_ELEMENT_ID "
	end if	
...
select case Request("Action")
	...
	case else		if Request("hdnExport") <> "" then
						...
						with objTxtStream
							...
							if hdnManagedObjects = "1" then
								.WriteLine "<TH>Network Element Name</TD>"
							end if
							...
							for k = 0 to UBound(aList, 2)
								...
								if hdnManagedObjects = "1" and strObjName = "" then
									.WriteLine "<TD NOWRAP>"&routineHtmlString(aList(7,k))&"</TD>"
								end if
...
<input type="hidden" name="txtCustServID" value="<%=strCustServID%>">
...
<input type="hidden" name="hdnManagedObjects" value>
...
<td align="left" valign="middle" colSpan="9">
	...
	<input type="submit" name="action" value="&gt;">
	<input type="submit" name="action" value="&gt;&gt;">&nbsp;
	<img SRC="images/excel.gif" onclick="document.frmCorrList.target='new';document.frmCorrList.hdnExport.value='xls';document.frmCorrList.hdnManagedObjects.value='0';document.frmCorrList.submit();document.frmCorrList.hdnExport.value='';document.frmCorrList.target='_self';" WIDTH="32" HEIGHT="32">
	<% if strObjName = "" then %>
	<img SRC="images/excel.gif" onclick="document.frmCorrList.target='new';document.frmCorrList.hdnExport.value='xls';document.frmCorrList.hdnManagedObjects.value='1';document.frmCorrList.submit();document.frmCorrList.hdnExport.value='';document.frmCorrList.target='_self';" WIDTH="32" HEIGHT="32">
	<% end if %>
