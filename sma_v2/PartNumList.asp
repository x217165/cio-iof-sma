...
<!--********************************************************************************************* Page name:	PartNumList.asp* Purpose:		To dynamically display the results of a search for an asset part number.*	* In Param:		This page reads following parameters*				txtDesc - this is the part number that is to be searched for (this was named criteris to part number cloning for model and part number easier)** Out Param:    The following fields get set in the first form of the calling detail screen:*               hdnID*				txtDesc** Created by:	Chris Roe Oct. 04, 2000*  ********************************************************************************************-->
<%
'check user's rights
Const ASP_NAME = "PartNumList.asp"
...
'build query
sql = " SELECT part_number_id" &_
	  " ,      part_number_desc" &_
	  " FROM   crp.part_number"

if strCriteria <> "" then
	sql = sql &	" WHERE  UPPER(part_number_desc) LIKE '" & routineOraString(strCriteria) & "%'"
end if

'order by part number description
sql = sql & " ORDER BY UPPER(part_number_desc)"
	...
	case else
			if Request("hdnExport") <> "" then
				...
				'Response.Write "</SCRIPT>"
					sql = "<script type=""text/javascript"">document.location=""export/"&strRealUserID&".xls"";</script>"
					Response.Write sql
					Response.End
...
<title>Asset Part Number Results</title>
<link href="stylesheets/styles.css" rel="stylesheet" type="text/css">
</head>
<script TYPE="TEXT/JAVASCRIPT">
function go_back(ID, Desc)
{
	parent.opener.document.forms[0].hdnPartNumID.value = ID;
	parent.opener.document.forms[0].txtPartNumDesc.value = Desc;
	parent.window.close ();
}
