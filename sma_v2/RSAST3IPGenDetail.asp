...
<!--
********************************************************************************************
* Page name:	RSAST3IPGenDetail.asp
* Purpose:		To display the detailed information about a Gateway IP address.
*				Gateway IP address chosen via RSAST3IPGenList.asp
* Created by:
* Updated by:	Dan Ty
********************************************************************************************
...
strWinLocation = "RSAST3IPGenDetail.asp?IPAddressID="& Request.Form("hdnIPAddressID")
strRealUserID = Session("username")

'get the customer contact id
lngIPAddressID = Request("hdnIPAddressID")

select case Request("hdnFrmAction")

	case "SAVE"

      'check to see if IP Address ID exists already in database by checking for a hidden IP Address ID
	  if Request.Form("hdnIPAddressID")  <> "" then  ' it is an existing record so save the changes
		...
		cmdUpdateObj.CommandText = "sma_sp_userid.spk_sma_rsas_inter.sp_cont_role_update"
		...
		cmdUpdateObj.Parameters.Append cmdUpdateObj.CreateParameter("p_ip_address_id", adNumeric, adParamInput,, Clng(Request("hdnCustomerContactID")))'the hidden customer contact id
...
'get requested customer_contact_id
	...
	if lngCustomerContactID <> 0 then
		...
		dim strContactInfo, strCustomerName ,strLName, strFName, lngContactID, strWorkFor, strContactName, strPosition, strWExt, strEmail, strBuilding, strStreet, strUserid
		...
		strContactInfo = "Works for:" & chr(9) & strWorkFor & chr(10) & "Position:" & chr(9) & strPosition & chr(10) & "UserID:" & chr(9) & strUserID & chr(10) & "Work # :" & chr(9) & strWP & " Ext: " & strWExt & chr(10) & "Cell # :" & chr(9) & strCP & chr(10) & "Pager # :" & chr(9) & strPP & chr(10) & "Fax # :" & chr(9) & strFP & chr(10) & "Email:" & chr(9) & strEmail & chr(10) & "Building:" & chr(9) & strBuilding & chr(10) & "Address:" & chr(9) & strStreet & chr(10) & chr(9) & strCPC & chr(10) & chr(9) & strPC & chr(10)
...
<FORM name=frmContactRoleDetail action="ContactRoleDetail.asp"  method="POST">
	...
	<INPUT name=hdnIPAddressID type=hidden value= <%if lngIPAddressID <> 0 then Response.Write lngIPAddressID else Response.Write """""" end if%>>
    ...
    <INPUT id=txtGatewayIP name=txtCustomerShortName type=hidden value= "">

	<!-- user interface -->
	<TABLE border=0 width=100%>
	<thead>
		<tr>
			<td colspan=3 align=left>Gateway IP Address ID Detail</td>
			</td>
	</thead>
	<TR>
		<TD align=right width=25%>Gateway IP Address<font color=red>*</font></TD>
		<TD align=left width=50% colspan=2>
			<input name=txtGatewayIPAddress type=text disabled size=50 maxlength=50 value="<%if lngCustomerContactID <> 0 then Response.Write routineHTMLString(strCustomerName) else Response.Write null end if%>" onChange="fct_onChange();">
	...
	<tr>
		...
		<td align=left width=50% colspan=2 disabled><textarea name=txtContactInfo cols=80 style="HEIGHT: 200px"><%if lngCustomerContactID <> 0 then Response.Write routineHTMLString(strContactInfo) else Response.Write null end if%></textarea></td>
	</tr>
