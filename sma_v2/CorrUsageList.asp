...
if isnumeric(strServTypeID)  then
	...
	'		" AND cs.CUSTOMER_SERVICE_ID = " & strCustServID
 	...
 	StrSql =" SELECT b.srvc_type_att_name, " &_
			...
			" AND  a.SRVC_TYPE_ATT_ID in  " &_
			" (select SRVC_TYPE_ATT_ID from crp.appl_srvc_typ_att_rule r, crp.appl_srvc_typ_att_rule_stat rs " &_
         	" where r.APPL_SRVC_TYP_ATT_RULE_ID=rs.APPL_SRVC_TYP_ATT_RULE_ID " &_
         	" and r.APPLICATION_ID = 1 " &_
			" and rs.APPL_SRVC_TYP_ATT_RULE_STAT_CD = 'A'  "&_
        	" and (rs.EFF_STOP_TS > sysdate or rs.EFF_STOP_TS = null)) " &_
			" AND  d.service_type_id = " & strServTypeID  &_
